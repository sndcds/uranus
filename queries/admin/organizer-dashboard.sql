...
WHERE uvl.user_id = $1
  AND (
    ur.edit_venue OR ur.delete_venue OR
    ur.add_space OR ur.edit_space OR ur.delete_space OR
    ur.add_event OR ur.edit_event OR ur.delete_event OR ur.release_event
    )
UNION
SELECT v.id AS venue_id
FROM {{schema}}.venue v
