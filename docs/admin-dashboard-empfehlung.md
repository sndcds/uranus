...
6. **Venue-Scope:** Authoritative ist `venue.scope`: `organization` bezeichnet einen organisationsgebundenen „Provisorischen Ort (nicht eigener Ort)“, `shared` einen eigenständigen/gemeinsam verwendbaren Datensatz („Eigener Ort“). Der Repository-Zielzustand ist `NOT NULL` mit CHECK auf genau diese Werte und **ohne Default**; jeder aktive Schreibpfad muss bei Erstellung einen gültigen Scope ausdrücklich liefern. Eine spätere Änderung ist über die aktuelle API nicht vorgesehen. Der Live-/Deployment-Zustand muss separat mit [Diagnose-SQL](../sql_utils/diagnose-venue-scope.sql) geprüft und gegebenenfalls migriert werden; keine automatische Umklassifizierung. [Befunde und Deployment-Reihenfolge](venue-scope.md).
