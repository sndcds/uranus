	...
	txErr := WithTransaction(ctx, h.DbPool, func(tx pgx.Tx) *ApiTxError {
		...
		if err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return &ApiTxError{
					...
					Err:  fmt.Errorf("No member with uuid %s found in organization with uuid: %s", memberUuid, orgUuid),
				}
			}
			return &ApiTxError{
		...
		if err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return &ApiTxError{
					Code: http.StatusNotFound,
					Err:  fmt.Errorf("No permissions found for user with uuid: %s in organization with uuid: %s", memberUserUuid, orgUuid),
				}
			}
			return &ApiTxError{
				Code: http.StatusInternalServerError,
