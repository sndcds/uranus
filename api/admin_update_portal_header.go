...
func (h *ApiHandler) AdminUpdatePortalHeader(gc *gin.Context) {
	apiRequest := grains_api.NewRequest(gc, "admin-update-portal-header")
	...
	query := fmt.Sprintf(`UPDATE %s.portal SET header = $1::jsonb WHERE uuid = $2::uuid`, h.DbSchema)
