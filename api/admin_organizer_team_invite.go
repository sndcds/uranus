...
func (h *ApiHandler) AdminOrganizerTeamInvite(gc *gin.Context) {
	...
	organizerSQL := fmt.Sprintf(`SELECT name FROM %s."organizer" WHERE id = $1`, h.Config.DbSchema)
	err = tx.QueryRow(ctx, organizerSQL, organizerId).Scan(&organizerName)
	if err != nil {
		if err == pgx.ErrNoRows {
	...
	insertSql := fmt.Sprintf(`
	           INSERT INTO %s.organizer_member_link (organizer_id, user_id, member_role_id, accept_token, invited_at, invited_by_user_id)
	           VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, $5)`,
		h.Config.DbSchema)
	_, err = pool.Exec(ctx, insertSql, organizerId, memberUserId, payload.RoleId, tokenString, userId)
	if err != nil {
	...
	emailMessage = strings.Replace(emailMessage, "{{organizer_name}}", organizerName, -1)
