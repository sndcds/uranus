...
func (h *ApiHandler) AdminUpdatePortalFooter(gc *gin.Context) {
	apiRequest := grains_api.NewRequest(gc, "admin-update-portal-footer")
	...
	query := fmt.Sprintf(`UPDATE %s.portal SET footer = $1::jsonb WHERE uuid = $2::uuid`, h.DbSchema)
