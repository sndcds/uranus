...
func applySocialPublicationMigration(t *testing.T, h *ApiHandler, direction string) {
	...
	if direction == "down" {
		// Roll back the newer worker schema before the Part 6 roundtrip.
		scheduling, err := os.ReadFile("../migrations/202609260002_social_scheduling.down.sql")
		if err != nil {
			t.Fatal(err)
		}
		dbExec(t, h, string(scheduling))
	}
	dbExec(t, h, string(sql))
	if direction == "up" {
		sql, err := os.ReadFile("../migrations/202609260002_social_scheduling.up.sql")
		if err != nil {
			t.Fatal(err)
		}
		dbExec(t, h, string(sql))
...
func TestSocialPublicationsPostgresHistoryAndIdempotency(t *testing.T) {
	...
	first := publishAndProcess(t, h, r, token, path+"/publish").Results[0]
	...
	second := publishAndProcess(t, h, r, token, path+"/publish").Results[0]
	...
	if target.RemotePostID == nil || *target.RemotePostID != "1" || target.PublishedAt == nil {
		t.Fatal("target does not reflect the matching historical success")
	}
...
func TestSocialPublicationsPostgresFailedRetry(t *testing.T) {
	...
	first := publishAndProcess(t, h, r, token, path).Results[0]
	...
	second := publishAndProcess(t, h, r, token, path).Results[0]
	...
	for _, outcome := range []string{"published", "failed"} {
		t.Run(outcome, func(t *testing.T) {
			...
			if outcome == "failed" {
				...
				next := publishAndProcess(t, h, r, token, path+"/publish").Results[0]
...
func TestSocialPublicationsPostgresFiltersAndPermissions(t *testing.T) {
	...
	post := previewPost(t, r, token, account)
	first := publishAndProcess(t, h, r, token, socialPostsPath+"/"+post.Uuid+"/publish").Results[0]
	...
	otherPublication := publishAndProcess(t, h, r, otherToken, socialPostsPath+"/"+otherPost.Uuid+"/publish").Results[0]
...
func TestSocialPublicationsPostgresSnapshotAfterClaim(t *testing.T) {
	...
	work, result, txErr := h.claimScheduledSocialTarget(context.Background())
	if txErr != nil || !work.claimed {
		t.Fatal("claim failed")
	}
	expected = work.rendered
	dbExec(t, h, "UPDATE uranus.event SET title='Changed after claim' WHERE uuid=$1", contentEvent)
	published, err := h.publishSocialTarget(context.Background(), work)
	if err != nil {
		t.Fatal(err)
	}
	result.Status, result.RemotePostID = "published", &published.RemotePostID
