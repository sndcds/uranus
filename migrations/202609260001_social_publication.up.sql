...
-- Part 5 has no rendered snapshot. Never reconstruct historical content from
-- today's source or infer whether a remote mutation succeeded.
INSERT INTO uranus.social_publication
    (uuid, social_post_uuid, social_post_target_uuid, social_account_uuid,
     platform, status, error, started_at, finished_at)
SELECT t.uuid, t.social_post_uuid, t.uuid, t.social_account_uuid,
    a.platform, 'uncertain', 'legacy Part 5 publishing target; rendered snapshot unavailable',
    t.updated_at, CURRENT_TIMESTAMP
FROM uranus.social_post_target t
JOIN uranus.social_account a ON a.uuid = t.social_account_uuid
